...
//        }
  ...
  private fun connectServer(callback: () -> Unit) {
    configs.videoStream = ReadStream(DatagramSocket(6006))
    configs.audioStream = ReadStream(DatagramSocket(7007))
  ...
  private fun setVideoDecodec() {
    ...
    // 获取视频标识头
    val csd0 = configs.videoStream.readFrame()
    val csd1 = configs.videoStream.readFrame()
  ...
  private fun setAudioDecodec(): Boolean {
    ...
    // 获取音频标识头
    mediaFormat.setByteBuffer("csd-0", ByteBuffer.wrap(configs.audioStream.readFrame()))
